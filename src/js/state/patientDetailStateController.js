 ...
 */
var patientInfo = {};
app.controller('patientDetailStateController', function ($timeout, $rootScope, $state, $scope, $location, $$log, $$title, getData, $$navbar, $$tabbar, $stateParams, $$confirm, $$toast, $http, $$requestUrl, $$loading) {
    ...
    $$log.debug($scope.dataArr.tag);
    $scope.data.tag = $scope.data.tag || [];
    $scope.data.departmentTag.tag = $scope.data.departmentTag.tag || [];
    //判断是否是子标签
    ...
    $scope.onIsSearchStatus = function (juge) {
        ...
        if ($scope.isSearch == false) {
            ...
            $('.trigger-spot').on('click', function () {
                $timeout(function () {
                    $('.searchInput').focus();
                }, 100);
