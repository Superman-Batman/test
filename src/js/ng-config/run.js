 ...
 */
var imLoaded = 'false';
app.run(function ($rootScope, $window, $location, $$log, $$env, $$initRem, $$wx, $$shence, $$loading, $$navbar, $state, $$txIM, $$tabbar, $timeout, $$oneAPM) {
    // $$log.hideAll(true);
    $$log.showAll(true);
    $$shence.init();
    // $$oneAPM.init();
    ...
    function stateChangeStart($rootScope, $state) {
        ...
        // }
        // }
        if (false && ($state.name == 'allPatientList' || $state.name == 'toolList') && imLoaded == 'false') {
            var listeners = {
        ...
        $$log.info($state);
        try {
            webModel.reachPage($state.name, location.hash);
        } catch (err) {
